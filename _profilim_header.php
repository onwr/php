<header class="normal">
    <div class="container">
        <div class="menu_header">
            <div class="logo">
                <img src="assets/image/logo.png" width="175" height="43" alt="Aktif Emlak Logo">
            </div>
            <ul class="menu_ul">
                <div class="img_area">
                    <a class="btn_menu_close openMenu"><img src="assets/image/icons/cancel.png" width="60" height="60" alt="Aktif Emlak Hamburger"></a>
                </div>
                <li class="menu_li active">
                    <a href="index.php" title="Ana Sayfa">Ana Sayfa</a>
                </li>
                <li class="menu_li">
                    <a href="satilik_ilanlar_liste.php" title="Satılık">Satılık</a>
                </li>
                <li class="menu_li">
                    <a href="#" title="Kiralık">Kiralık</a>
                </li>
                <li class="menu_li">
                    <a href="#" title="Konut">Konut</a>
                </li>
                <li class="menu_li">
                    <a href="#" title="Arsa">Arsa</a>
                </li>
                <li class="menu_li">
                    <a href="#" title="Villa">Villa</a>
                </li>
                <li class="menu_li">
                    <a href="#" title="İşyeri">İşyeri</a>
                </li>
                <li class="menu_li">
                    <a href="kurumsal.php" title="Kurumsal">Kurumsal</a>
                </li>
                <li class="menu_li">
                    <a href="iletisim.php" title="İletişim">İletişim</a>
                </li>
            </ul>
            <ul class="menu_buttons">
                <li class="active_li">
                    <a href="uyelik_kullanici_bilgileri.php" title="Profilim" class="active_kayit">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"  style="margin-right: 10px;">
                        <g clip-path="url(#clip0_4609_18405)">
                            <path d="M24 11.988C24 5.37 18.624 0 12 0C5.376 0 0 5.37 0 11.988C0 15.633 1.656 18.918 4.248 21.123C4.272 21.147 4.296 21.147 4.296 21.171C4.512 21.339 4.728 21.507 4.968 21.675C5.088 21.747 5.184 21.8415 5.304 21.9375C7.28791 23.2804 9.62834 23.9987 12.024 24C14.4197 23.9987 16.7601 23.2804 18.744 21.9375C18.864 21.8655 18.96 21.771 19.08 21.6975C19.296 21.531 19.536 21.363 19.752 21.195C19.776 21.171 19.8 21.171 19.8 21.147C22.344 18.9165 24 15.633 24 11.988ZM12 22.4895C9.744 22.4895 7.68 21.7695 5.976 20.571C6 20.379 6.048 20.1885 6.096 19.9965C6.23948 19.4763 6.4492 18.9767 6.72 18.51C6.984 18.054 7.296 17.646 7.68 17.286C8.04 16.926 8.472 16.5915 8.904 16.3275C9.36 16.0635 9.84 15.8715 10.368 15.7275C10.9003 15.5849 11.449 15.5128 12 15.513C13.6361 15.5006 15.2123 16.1283 16.392 17.262C16.944 17.814 17.376 18.4615 17.688 19.2045C17.856 19.6365 17.976 20.092 18.048 20.571C16.2768 21.8162 14.1652 22.4861 12 22.4895ZM8.328 11.3895C8.11697 10.9052 8.01064 10.3818 8.016 9.8535C8.016 9.327 8.112 8.799 8.328 8.319C8.544 7.839 8.832 7.4085 9.192 7.0485C9.552 6.6885 9.984 6.402 10.464 6.186C10.944 5.97 11.472 5.874 12 5.874C12.552 5.874 13.056 5.97 13.536 6.186C14.016 6.402 14.448 6.69 14.808 7.0485C15.168 7.4085 15.456 7.8405 15.672 8.319C15.888 8.799 15.984 9.327 15.984 9.8535C15.984 10.4055 15.888 10.9095 15.672 11.388C15.4647 11.8616 15.1718 12.2928 14.808 12.66C14.4407 13.0233 14.0094 13.3157 13.536 13.5225C12.5441 13.9293 11.4319 13.9293 10.44 13.5225C9.96656 13.3157 9.53534 13.0233 9.168 12.66C8.80387 12.298 8.51796 11.8665 8.328 11.3895ZM19.464 19.3485C19.464 19.3005 19.44 19.2765 19.44 19.2285C19.2044 18.4775 18.8564 17.7664 18.408 17.1195C17.9592 16.4679 17.4081 15.893 16.776 15.417C16.2931 15.0536 15.7696 14.7475 15.216 14.505C15.4667 14.3373 15.6998 14.1448 15.912 13.9305C16.2698 13.5773 16.584 13.1824 16.848 12.7545C17.3814 11.882 17.656 10.876 17.64 9.8535C17.6478 9.09673 17.5008 8.34636 17.208 7.6485C16.9192 6.97598 16.5037 6.36536 15.984 5.85C15.4638 5.34157 14.8534 4.93462 14.184 4.65C13.4851 4.35728 12.7337 4.21077 11.976 4.2195C11.2182 4.21125 10.4668 4.35827 9.768 4.6515C9.09127 4.93264 8.47883 5.34859 7.968 5.874C7.45692 6.39145 7.0496 7.00193 6.768 7.6725C6.47517 8.37036 6.32816 9.12073 6.336 9.8775C6.336 10.4055 6.408 10.909 6.552 11.388C6.696 11.892 6.888 12.348 7.152 12.7785C7.392 13.2105 7.728 13.5945 8.088 13.9545C8.304 14.1705 8.544 14.362 8.808 14.529C8.25195 14.7765 7.72809 15.0909 7.248 15.465C6.624 15.945 6.072 16.5195 5.616 17.1435C5.16341 17.788 4.81514 18.4997 4.584 19.2525C4.56 19.3005 4.56 19.3485 4.56 19.3725C2.664 17.454 1.488 14.865 1.488 11.988C1.488 6.21 6.216 1.4865 12 1.4865C17.784 1.4865 22.512 6.21 22.512 11.988C22.5089 14.7479 21.413 17.3943 19.464 19.3485Z" fill="#F4F4F4"/>
                        </g>
                        <defs>
                            <clipPath id="clip0_4609_18405">
                            <rect width="24" height="24" fill="white"/>
                            </clipPath>
                        </defs>
                        </svg>    
                        <span>Profilim</span>
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18" fill="none" style="margin-left: 5px;">
                            <g clip-path="url(#clip0_4609_18408)">
                                <path fill-rule="evenodd" clip-rule="evenodd" d="M9.52831 11.7803C9.38766 11.9209 9.19693 11.9999 8.99806 11.9999C8.79919 11.9999 8.60846 11.9209 8.46781 11.7803L4.22506 7.53751C4.15343 7.46833 4.09629 7.38557 4.05698 7.29407C4.01768 7.20256 3.99699 7.10415 3.99612 7.00456C3.99526 6.90498 4.01423 6.80622 4.05194 6.71405C4.08965 6.62188 4.14534 6.53814 4.21576 6.46772C4.28618 6.3973 4.36992 6.34161 4.46209 6.3039C4.55427 6.26619 4.65303 6.24721 4.75261 6.24808C4.8522 6.24894 4.95061 6.26963 5.04211 6.30894C5.13362 6.34824 5.21637 6.40538 5.28556 6.47701L8.99806 10.1895L12.7106 6.47701C12.852 6.34039 13.0415 6.2648 13.2381 6.26651C13.4348 6.26822 13.6229 6.34709 13.7619 6.48615C13.901 6.6252 13.9799 6.81331 13.9816 7.00996C13.9833 7.20661 13.9077 7.39606 13.7711 7.53751L9.52831 11.7803Z" fill="#F4F4F4"/>
                            </g>
                            <defs>
                                <clipPath id="clip0_4609_18408">
                                <rect width="18" height="18" fill="white"/>
                                </clipPath>
                            </defs>
                        </svg>
                    </a>
                    <ul class="hover_open">
                            <li>
                                <a href="uyelik_kullanici_bilgileri.php">Kullanıcı Bilgileri</a>
                            </li>
                            <li>
                                <a href="uyelik_favori_ilanlarim.php">Favori İlanları</a>
                            </li>
                            <li>
                                <a href="uyelik_favori_aramalar.php">Favori Aramalar</a>
                            </li>
                            <li>
                                <a href="uyelik_ilan_karsilastirma.php">İlan Karşılaştırma</a>
                            </li>
                            <li>
                                <a href="uyelik_kullanici_bilgileri.php">İlan Karşılaştırma</a>
                            </li>
                            <li>
                                <a href="#">Mesaj</a>
                            </li>
                            <li>
                                <a href="#">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="25" height="24" viewBox="0 0 25 24" fill="none">
                                        <path d="M5.5 21C4.95 21 4.47933 20.8043 4.088 20.413C3.69667 20.0217 3.50067 19.5507 3.5 19V5C3.5 4.45 3.696 3.97933 4.088 3.588C4.48 3.19667 4.95067 3.00067 5.5 3H12.5V5H5.5V19H12.5V21H5.5ZM16.5 17L15.125 15.55L17.675 13H9.5V11H17.675L15.125 8.45L16.5 7L21.5 12L16.5 17Z" fill="#011E2E"/>
                                    </svg>
                                    <span>Çıkış Yap</span>
                                </a>
                            </li>
                        </ul>
                </li>
            </ul>

            <a href="#" class="menu_open_button openMenu">
                <img src="assets/image/icons/hamburger.png" width="25" height="25" alt="Aktif Emlak Hamburger">
            </a>
        </div>
    </div>
</header>