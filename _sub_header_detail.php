<header class="normal">
    <div class="container">
        <div class="menu_header">
            <div class="geri_don_button d-md-none d-flex">
                <img src="assets/image/icons/okok.png" width="7" height="12" alt="Aktif Emlak Logo">
                <span>Geri Dön</span>
            </div>
            <div class="logo">
                <img src="assets/image/logo-white.png" width="175" height="43" alt="Aktif Emlak Logo">
            </div>
            <ul class="menu_ul">
                <div class="img_area">
                    <a class="btn_menu_close openMenu"><img src="assets/image/icons/cancel.png" width="60" height="60" alt="Aktif Emlak Hamburger"></a>
                </div>
                <img src="assets/image/logo-is-logo.png" style="width: 100%;" class="d-xl-none d-block" alt="">
                <li class="menu_li active">
                    <a href="index.php" title="Ana Sayfa">Ana Sayfa <span>></span></a>
                </li>
                <li class="menu_li">
                    <a href="satilik_ilanlar_liste.php" title="Satılık">Satılık <span>></span></a>
                </li>
                <li class="menu_li">
                    <a href="#" title="Kiralık">Kiralık <span>></span></a>
                </li>
                <li class="menu_li">
                    <a href="#" title="Konut">Konut <span>></span></a>
                </li>
                <li class="menu_li">
                    <a href="#" title="Arsa">Arsa <span>></span></a>
                </li>
                <li class="menu_li">
                    <a href="#" title="Villa">Villa <span>></span></a>
                </li>
                <li class="menu_li">
                    <a href="#" title="İşyeri">İşyeri <span>></span></a>
                </li>
                <li class="menu_li">
                    <a href="kurumsal.php" title="Kurumsal">Kurumsal <span>></span></a>
                </li>
                <li class="menu_li">
                    <a href="iletisim.php" title="İletişim">İletişim <span>></span></a>
                </li>
            </ul>
            <ul class="menu_buttons d-md-flex d-none">
                <li>
                    <a href="giris-yap.php" title="Giriş">Giriş</a>
                </li>
                <li>
                    <a href="yeni-uyelik.php" title="Kayıt Ol" class="active_kayit">Kayıt Ol</a>
                </li>
            </ul>
            <a href="#" class="menu_open_button openMenu">
                <img src="assets/image/icons/hamburger.png" width="25" height="25" alt="Aktif Emlak Hamburger">
            </a>
        </div>
    </div>
</header>